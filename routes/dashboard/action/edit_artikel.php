...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($judul && $konten && !$error) {
        $stmt = mysqli_prepare($koneksi, "UPDATE artikel SET title = ?, body = ?, thumbnail = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "sssi", $judul, $konten, $fotoPath, $id);
        if (mysqli_stmt_execute($stmt)) {
            $success = "Data berhasil diperbarui.";
            // Update variabel agar form ikut diperbarui
            $data["title"] = $judul;
            $data["body"] = $konten;
            $data["thumbnail"] = $fotoPath;
        } else {
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-white active" aria-current="page">Edit Artikel</li>
                    </ol>
